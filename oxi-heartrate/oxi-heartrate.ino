...
#define NUMPIXELS 1       // Samo jedna led dioda

// Inicijalizacija NeoPixel: broj dioda, pin, tip signala
// Redosled boja je GRB, a ne RGB
...
// Binarni paket se salje na 2 sekunde
const int sendInterval = 2000;

// ================= SENZOR PODESAVANJA =================

// __attribute((packed)) -> sprecava da kompajler dodaje padding bajtove izmedju polja,
// cime garantujemo da paket ima tacno 14 bajtova

// sub.py raspakuje sa 'struct.unpack("<iiibb", payload)
...
// Izlazne promenljive koje algoritam popunjava
int32_t spo2 = 0; 
int8_t validSPO2 = 0; 
int32_t heartRate = 0; 
...
#define GRAPH_MID (GRAPH_Y + (GRAPH_H / 2))   // Sredisnja linija grafa (y koordinata)
static int16_t y_staro = GRAPH_MID; 

// Za potrebe crtanja grafa na displeju
long sredisnjaVrednost = 250000; 

static int32_t prikazaniBPM = 0;    // Poslednji BPM koji se prikazuje na ekranu
static int32_t prikazaniSPO2 = 0;   // Poslednji SpO2 koji se prikazuje na ekranu
...
static uint32_t lastDrawTime = 0;   // Vreme poslednjeg osvezavanja grafa

// ================= PRAVLJENJE KLASE ZA ESP32C6 - LCD =================

// Nasledjujemo lgfx::LGFX_Device i rucno namestano sve parametre
// LovyanGFX nema ugradjenu podrsku za ovu kombinaciju
...
class LGFX_ESP32C6 : public lgfx::LGFX_Device // Klasa za nas uredjaj LGFX_ESP32C6
{
  ...
  lgfx::Light_PWM    _light_instance;         // PWM kontrola pozadinskog osvetljenja       
  ...
  public:
    LGFX_ESP32C6(){     // konstruktor i konfiguracija/namestanje raznih podesavanja
      ...
      bus_cfg.spi_host = SPI2_HOST;           // SPI1 je rezervisan interno za Flash memoriju cipa, pa se koristi SPI2

      bus_cfg.spi_mode = 0;                   // Rezim rada - tacan trenutak takta - podaci se uzorkuju na rastucoj ivici takta
      ...
      bus_cfg.dma_channel = SPI_DMA_CH_AUTO;  // Postavlja DMA kanal koji ce koristiti SPI_DMA_CH_AUTO = automatsko podesavanje,  
                                              // DMA - Direct Memory Access, CPU odmara, DMA salje sliku na ekran u pozadini

      // Mapiranje pinova, gpio
      bus_cfg.pin_sclk = 7;              // Taktni signal
      bus_cfg.pin_mosi = 6;              // Podaci na ekranu
      ...
      panel_cfg.dummy_read_pixel = 8;     // Ostavi 8 (standardno za ST7789 drajver cip)
      ...
      light_cfg.invert = false;     // HIGH -> svetlo ukljuceno
...
void setup() {
  ...
  // ================= PODESAVANJE ZA SENZOR =================
  if (!particleSensor.begin(Wire, I2C_SPEED_FAST)) {
    Serial.println("MAX30105 nije pronadjen!");
    while (1);
  }
  ...
  byte sampleRate = 100;    
  int pulseWidth = 411;       // Sirina pulsa LED-a u ms (veca -> vise svetla, ali sporije)   
  int adcRange = 4096;      

  particleSensor.setup(ledBrightness, sampleAverage, ledMode, sampleRate, pulseWidth, adcRange);

  // ================= PODESAVANJE ZA ESP DISPLEJ =================
  ...
  display.fillScreen(TFT_BLACK);   //Ooboji ekran u crno

  // Inicijalni tekst na ekranu pre prvog merenja
...
void loop() {
  ...
  // Citamo sirove vrednosti direktno sa senzora
  long irValue = particleSensor.getIR();
  long redValue = particleSensor.getRed();

  // KONTINUALNO CRTANJE GRAFIKA:
  // Crta se jedna tacka u svakom prolazu kroz loop
  ...
  if (irValue < 20000) { 
    ...
    if (!bufferFull) {           // Ako nije pun ne vrsi se merenje
      // Punjenje bafera sa 100 uzoraka -> algoritam ne moze da racuna dok nema dovoljno podataka
  ...
  // ================= MQTT SLANJE PODATAKA (BINARNO) I OSVEZAVANJE EKRANA =================
  long now = millis();
  if (now - lastMsg > sendInterval) {
    ...
    // Za pamcenje prethodnih vrednosti
    ...
    paket.ir = irValue;   // Ukljucujemo i sirovu IR vrednost

    // Objavljivanje binarnog paketa na MQTT
    if (irValue < 20000) {
      paket.bpm = 0;
      paket.spo2 = 0;
      paket.validBPM = 0;
...
// MQTT callback za dolazece poruke, nije nam potreban, jer ESP32 salje podatke
// PubSubClient zahteva da callback bude registrovan

void reconnect() {
  while (!client.connected()) {
    Serial.print("Pokusavam MQTT povezivanje...");

    // Generisemo novi random Client ID da izbegnemo konflikte
    // Ovo radimo, zato sto broker odbija konekciju ako vec postoji klijent sa istim ID-jem
...
void crtajTackuNaEkranu(long irValue) {
  if (irValue < 20000) {
    // Nema prsta -> brise se kolona i crta se crvena sredisnja linija
  ...
  // Sporija reakcija = stabilnija sredina
  ...
  // Brisemo kolonu ispred trenutne pozicije da napravimo efekat pomeranja grafa
  display.fillRect(grafPozicija + 1, GRAPH_Y, 8, GRAPH_H, TFT_BLACK);

  if (grafPozicija > 0) {
    // Osnovna linija izmedju tacaka (crtamo liniju od prethodne do trenutne tacke)
    display.drawLine(grafPozicija - 1, y_staro, 
                     grafPozicija, y_trenutno, TFT_GREEN);
    
    // Vertikalno popuni sve piksele izmedju y_staro i y_trenutno na trenutnoj X poziciji
  ...
  // Pamtimo trenutnu Y vrednost za sledeci prikaz
  y_staro = y_trenutno;
  grafPozicija++;

  // Kada stignemo do desnog kraja ekrana, vracamo se na levu stranu
  if (grafPozicija >= GRAPH_W) grafPozicija = 0;
}